...
const { sysConfig } = require('../allconfigs');
 ...
 * Room class.
 *
 * This is not a "mediasoup Room" by itself, by a custom 
 * class that holds a protoo Room (for signaling with 
 * WebSocket clients) and a mediasoup Router (for sending 
 * and receiving media to/from those WebSocket peers).
 */
class Room extends EventEmitter {
	 ...
	 * @async
	 *
	 * @param {mediasoup.Worker} mediasoupWorker - The mediasoup 
     *   Worker in which a new mediasoup Router must be created.
    ...
    static async create({ mediasoupWorker, roomId }) {
		...
		// Router media codecs.
		const { mediaCodecs } = sysConfig.routerOptions;
